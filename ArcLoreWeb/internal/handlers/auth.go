...
// identity_token, token_expiry, is_admin) in the scs session via
// auth.StoreIdentity; identity_token is the Bearer forwarded onward to both
// lore (via the exchange) and the mgmt API.
type Native struct {
	Mgmt     *mgmt.Client
...
func (n *Native) LoginForm(w http.ResponseWriter, r *http.Request) {
	...
	n.renderAuthPage(w, r, "Sign in — ArcLore", templates.LoginPage("", status.RegistrationOpen))
}
...
func (n *Native) LoginSubmit(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		n.renderAuthPage(w, r, "Sign in — ArcLore", templates.LoginPage("Invalid form submission.", n.registrationOpen(r)))
		return
	}
	...
	if err != nil {
		...
		if errors.As(err, &apiErr) && apiErr.Status == http.StatusUnauthorized {
			n.renderAuthPage(w, r, "Sign in — ArcLore", templates.LoginPage("Invalid username or password.", n.registrationOpen(r)))
			return
		}
		n.renderAuthPage(w, r, "Sign in — ArcLore", templates.LoginPage("Sign-in is unavailable. Try again shortly.", n.registrationOpen(r)))
		return
	}

	if !auth.StoreIdentity(n.Sessions, w, r, resp.UserID, resp.Name, resp.Token, resp.ExpiresAt, resp.IsAdmin) {
		return
	}
...
func (n *Native) RegisterForm(w http.ResponseWriter, r *http.Request) {
	...
	n.renderAuthPage(w, r, "Register — ArcLore", templates.RegisterPage(""))
}
...
func (n *Native) RegisterSubmit(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		n.renderAuthPage(w, r, "Register — ArcLore", templates.RegisterPage("Invalid form submission."))
		return
	}
	username := r.PostFormValue("username")
	password := r.PostFormValue("password")
	...
	if err != nil {
		...
		if errors.As(err, &apiErr) {
			...
			n.renderAuthPage(w, r, "Register — ArcLore", templates.RegisterPage(msg))
			return
		}
		n.renderAuthPage(w, r, "Register — ArcLore", templates.RegisterPage("Registration is unavailable. Try again shortly."))
		return
	}
...
func (n *Native) AccountForm(w http.ResponseWriter, r *http.Request) {
	n.renderAuthPage(w, r, "Account — ArcLore", templates.AccountPage("", ""))
}
...
func (n *Native) ChangePassword(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		n.renderAuthPage(w, r, "Account — ArcLore", templates.AccountPage("Invalid form submission.", ""))
		return
	}
	...
	if err := n.Mgmt.ChangeMyPassword(r.Context(), identity.Token, current, newPassword); err != nil {
		...
		if errors.As(err, &apiErr) {
			...
			n.renderAuthPage(w, r, "Account — ArcLore", templates.AccountPage(msg, ""))
			return
		}
		n.renderAuthPage(w, r, "Account — ArcLore", templates.AccountPage("Password change is unavailable. Try again shortly.", ""))
		return
	}
	n.renderAuthPage(w, r, "Account — ArcLore", templates.AccountPage("", "Password changed."))
}
...
func (n *Native) SetupForm(w http.ResponseWriter, r *http.Request) {
	status, err := n.Mgmt.Status(r.Context())
	if err != nil {
		n.renderAuthPage(w, r, "Set up ArcLore", templates.SetupPage("Could not reach the auth service. Is it running?"))
		return
	}
	if status.HasUsers {
		http.Redirect(w, r, "/auth/login", http.StatusSeeOther)
	...
	n.renderAuthPage(w, r, "Set up ArcLore", templates.SetupPage(""))
}
...
func (n *Native) SetupSubmit(w http.ResponseWriter, r *http.Request) {
	if err := r.ParseForm(); err != nil {
		n.renderAuthPage(w, r, "Set up ArcLore", templates.SetupPage("Invalid form submission."))
		return
	}
	username := r.PostFormValue("username")
	password := r.PostFormValue("password")
	...
	if err != nil {
		...
		if errors.As(err, &apiErr) {
			...
			n.renderAuthPage(w, r, "Set up ArcLore", templates.SetupPage(apiErr.Message))
			return
		}
		n.renderAuthPage(w, r, "Set up ArcLore", templates.SetupPage("Could not reach the auth service. Is it running?"))
		return
...
// renderAuthPage wraps a body in the base Layout and writes a 200. When the
// request is an htmx partial swap, only the body fragment is sent. It renders
// with an EMPTY identity AND empty section so the login/setup/register/account
// pages show no nav pill and no logout affordance — preserving the exact
// behavior of the old auth.Native.renderPage. It deliberately does NOT route
// through renderPageStatus, which would compute a section from the path and
// wrongly light the Home pill on auth pages.
func (n *Native) renderAuthPage(w http.ResponseWriter, r *http.Request, title string, body templ.Component) {
